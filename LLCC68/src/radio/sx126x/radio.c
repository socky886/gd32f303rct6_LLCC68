...
SX126x_t SX126x=
{
    .Spi.Nss.pinIndex=RADIO_NSS,
    .Reset.pinIndex=RADIO_RESET,
    .BUSY.pinIndex=RADIO_BUSY,
    .DIO1.pinIndex=RADIO_DIO1,
};
...
bool RadioIsChannelFree( uint32_t freq, uint32_t rxBandwidth, int16_t rssiThresh, uint32_t maxCarrierSenseTime )
{
    ...
    while( carrierSenseTime < maxCarrierSenseTime )
    {
        DelayMs(1);
        carrierSenseTime++;
...
void RadioIrqProcess( void )
{
    ...
    //if( isIrqFired == true )
    {
        // printf("execute radio IRQ process\n");
